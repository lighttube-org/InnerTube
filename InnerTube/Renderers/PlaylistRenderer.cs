...
public class PlaylistRenderer : IRenderer
{
	...
	public PlaylistRenderer(JToken renderer)
	{
		...
		Thumbnails = Utils.GetThumbnails(renderer.GetFromJsonPath<JArray>("thumbnailRenderer.playlistVideoThumbnailRenderer.thumbnail.thumbnails")!);
		VideoThumbnails = new Dictionary<string, IEnumerable<Thumbnail>>();
		foreach (JArray thumbnails in renderer["thumbnails"]!.ToObject<JArray>()!.Select(x => x["thumbnails"]!.ToObject<JArray>()!))
		{
		...
		Videos = Utils.ParseRenderers(renderer["videos"]!.ToObject<JArray>()!).Cast<ChildVideoRenderer>();
	}
	...
	public override string ToString()
	{
		StringBuilder sb = new StringBuilder()
			...
			.AppendLine($"- VideoThumbnails count: {VideoThumbnails.Keys.Count} - {string.Join(", ", VideoThumbnails.Select(x => x.Key + ": " + x.Value.Count()))}")
