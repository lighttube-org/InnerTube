...
[ValueParser("fr-CA")]
public partial class CanadianFrench : IValueParser
{
	...
	public string ParseRelativeDate(string date)
	{
		...
		if (parts[3].Contains('\u00a0'))
		{
			...
			metric = nbspParts[1].TrimEnd('s');
			amount = int.Parse(nbspParts[0]);
		}
		else
		{
			metric = parts[4].TrimEnd('s');
	...
	public DateTimeOffset ParseFullDate(string date)
	{
		...
		return DateTimeOffset.ParseExact(match.Groups[1].Value, "d MMM yyyy", new CultureInfo("fr-CA"));
	}
	...
	public VideoUploadType ParseVideoUploadType(string type)
	{
		return type.Split(' ')[0] switch
		{
			...
			"Diffusé" => VideoUploadType.Streamed,
	...
	public long ParseViewCount(string viewCountText) => int.Parse(string.Join("", viewCountText.Where(char.IsDigit)));
